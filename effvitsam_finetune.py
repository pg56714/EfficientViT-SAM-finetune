...
def main():
    ...
    sam_model = create_sam_model(name="xl1", weight_url="./checkpoints/sam/xl1.pt").to(device).train()
    ...
    for epoch in range(num_epochs):
        ...
        for i, img_path in enumerate(img_all_pbar):
            ...
            upscaled_masks = torch.nn.functional.interpolate(
                upscaled_masks, size=gt_binary_mask.shape[-2:], mode='bilinear', align_corners=False
            )
            ...
            if i % 100 == 0:
                ...
                image_save_resized = cv2.resize(image_save, (mask_save.shape[1], mask_save.shape[0]))
